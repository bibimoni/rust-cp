...
fn main() {
  ...
  let mut amari: Vec<Vec<(usize, usize, usize)>> = vec![vec![]; n + 1];
  for i in 1..=m {
    let (u, v): (usize, usize) = (scan.next(), scan.next());
    if uf.is_same_set(u, v) {
      amari[uf.root(u)].push((u, v, i));
    } else {
      uf.unite(u, v);
    }
  }
  ...
  for i in 1..=n {
    if uf.root(i) == i {
      cc.push((amari[i].len(), i));
    }
  }
  cc.sort_by(|a, b| b.0.cmp(&a.0));
  writeln!(writer, "{}", cc.len() - 1).ok();
  let mut to = 1;
  for &(_, root) in cc.iter() {
    for &(start, _, idx) in amari[root].iter() {
      if to < cc.len() {
        writeln!(writer, "{} {} {}", idx, start, cc[to].1).ok();
        to += 1;
      }
    }
  }
