...
fn main() {
  ...
  for _ in 0..tt {
    ...
    let m_1 = scan.next::<usize>();
    let m_2 = scan.next::<usize>();

    let mut ef: Vec<(usize, usize)> = vec![];
    for _ in 0..m_1 {
    ...
    let mut g = union_find::UnionFind::new(n);
    for _ in 0..m_2 {
      let u = scan.next::<usize>() - 1;
      let v = scan.next::<usize>() - 1;
      g.unite(u, v);
    ...
    let mut f = union_find::UnionFind::new(n);
    let mut rem_edges = 0;
    for (u, v) in ef {
      if (g.is_same_set(u, v)) {
        f.unite(u, v);
      } else {
        rem_edges += 1;
      }
    ...
    writeln!(writer, "{}", rem_edges + f.size() - g.size()).ok();
  }
}
